...
bool brd[5][5][5];
pair<int, int> dist[5][5][5]; // 0으로 초기화 된 상태에서 별도로 무언가를 하지 않기 때문에 시작점에 한 번 더 들어가기는 하나, 미묘한 차이라서 그냥 놔둠. -1로 초기화하는 게 bfs의 관점에서만 바라보면 더 안전한 방법임.
                ...
                for (int k = 0; k < 5; k += 4) {
                    ...
                    while(!Q.empty()) {
                        ...
                        for (int dir = 0; dir < 6; dir++) {
                            ...
                            if (!brd[pile[nx]][ny][nz] || dist[pile[nx]][ny][nz].first == perm) continue;
                            
                            Q.push({nx, ny, nz});
                            dist[pile[nx]][ny][nz] = {perm, dist[pile[X]][Y][Z].second + 1};
                        }
                    }
                    
                    if (dist[pile[4]][4-j][4-k].first == perm && dist[pile[4]][4-j][4-k].second) {
                        if (ans < 0) ans = dist[pile[4]][4-j][4-k].second;
                        else ans = min(ans, dist[pile[4]][4-j][4-k].second);
                    }
