...
// 해결하려면 벡터 써야함. brd에 도착지를 표시하면 안됨. (출발지는 표시해도 될 듯)
// 문제점인지는 모르겠지만 실패 조건문을 조금 바꿔볼 예정 며칠 째 문제를 못 찾는 중
#include <bits/stdc++.h>
using namespace std;
#define X first
#define Y second
...
int main() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        V.push_back({tx2, ty2}); // brd2의 n번 칸은 n번 손님의 도착지
    } // 문제점 발견 : 다른 손님끼리 도착지가 겹칠 수 있음. (2번 손님과 3번 손님의 도착지가 1,1인 동시에 1번 손님의 출발지가 1,1일 수가 있음)
    ...
    for (int i = 0; i < M; i++) {
        ...
        while(!Q2.empty()) {
            ...
            if (cur.X == V[guest-3].X && cur.Y == V[guest-3].Y) {
                Q.push({cur.X, cur.Y});
                dist[cur.X][cur.Y] = 0;
                F -= dist2[cur.X][cur.Y];
                gift = dist2[cur.X][cur.Y] * 2;
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                if (nx < 1 || nx > N || ny < 1 || ny > N) continue;
                if (brd[nx][ny] == 1 || dist2[nx][ny] >= 0) continue;
