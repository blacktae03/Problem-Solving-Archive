...
// 내가 처음에 할려고 했던 방법인데 다음칸이 더 낮을 때를 구현하기가 어려워서 포기했던 걸로 기억함.
// line이라는 벡터가 굳이 필요한가 싶은데, 가로가 아니라 세로로 비교할 때는 check 함수가 일관성이 떨어져서 벡터로 처리한 거 같음.
// check 함수의 else 부분을 잘 검토하고 넘어가자. 나의 실패지점.
#include <bits/stdc++.h>
using namespace std;
